...
```{r, warning=FALSE, message=FALSE, cache=TRUE, eval=TRUE, results=FALSE}
tcga_data <- eh[["EH1661"]]
TCGA_bs <- eh[["EH1662"]]
...
Phenotypic data can be obtained using *pData* function in *Biobase*
```{r, warning=FALSE, message=FALSE, eval=TRUE}
library(bsseq)
phenoData <- bsseq::pData(TCGA_bs)
head(phenoData)
```


Methylation Comparison between normal and tumor samples
```{r, warning=FALSE, message=FALSE, eval=TRUE, fig.height=5, fig.width=4}
library(ggplot2)
...
g <- g + ggplot2::xlab('sample type') + ggplot2::ylab('Mean methylation') 
g <- g + ggplot2::theme(axis.text.x = element_text(angle = 0, hjust = 1))
g <- g + ggplot2::ggtitle('Mean methylation comparison between 
...
```{r, warning=FALSE, message=FALSE, eval=TRUE, fig.height=6, fig.width=5}
# In this analysis we restrict the data to just chromosome 22
chrIndex <- seqnames(TCGA_bs) == 'chr22'

# Also we are only conducting the analysis for 3 normal, tumor pairs
...
```{r, warning=FALSE, message=FALSE, eval=TRUE, fig.height=3.5, fig.width=7}
